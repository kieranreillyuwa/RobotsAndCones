...
processDataPackets(PhidgetChannelHandle ch, const uint8_t *buffer, size_t length) {
	...
	RXErrorCode errorCode = RX_ERROR_OK;
...
PhidgetDataAdapterSupport_dataInput(PhidgetChannelHandle ch, const uint8_t *buffer, size_t length) {
	...
	switch (ch->UCD->uid) {
	case PHIDCHUID_ADP_RS232_DATAADAPTER_100_USB:
	case PHIDCHUID_ADP_RS232_DATAADAPTER_100_VINT:
...
PhidgetReturnCode sendDataBuffer(PhidgetChannelHandle ch, size_t len, const uint8_t *buffer, BridgePacket* bp, int waitResposne) {
	...
	if (dataAdapterSupport->txTimeout == 0)
		maxDuration = ((double)len / DATAADAPTER_SUPPORT(ch)->baudRate) * 2000000 * 8 + 100000; //2 * transmission time in us + 100ms
	else
...
PhidgetReturnCode
PhidgetDataAdapterSupport_create(PhidgetDataAdapterSupportHandle *ir) {

	TESTPTR_PR(ir);
	*ir = mos_zalloc(sizeof(PhidgetDataAdapterSupport));
